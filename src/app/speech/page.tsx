...
export default function Home() {
  ...
  async function getGeneratedVoices() {
    const getResponse = await fetch("/api/speech", {
      method: "GET",
    });
  ...
  return (
    <main className="flex min-h-screen bg-gray-900 flex-col items-center justify-between px-24 py-12">
      ...
      <div className="mb-4">
        This is a project that uses the ElevenLabs API to convert your text into a
            ...
            {arrayOfVoices ? (
              <div className="flex gap-4">
                {arrayOfVoices.map((e: VoiceObject) => (
                  <button
                    className={`border ${theVoiceId === e.voice_id ? "bg-green-700" : ""
